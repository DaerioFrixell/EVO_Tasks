...
input.addEventListener("input", function (e) {
	if (e?.target?.value) {
		...
		if (value[3] + value[4] === "00") {
			input.value = value.slice(0, 3) + "01";
		}

		if (Number(value[3] + value[4]) > 12) {
			input.value = value.slice(0, 3) + "12";
		}

		if (value[6] === "0") {
			input.value = value.slice(0, 6) + "1";
		}

		if (value[6] === "1" && Number(value[7]) < 9) {
			input.value = value.slice(0, 7) + "9";
		}

		if (value[6] + value[7] === "19" && Number(value[8]) < 6) {
			input.value = value.slice(0, 8) + "6";
		}
